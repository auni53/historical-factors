...
def do(label):
  print "splitting %s" % label
  command = "split --bytes 500M --numeric-suffixes --suffix-length=3 google-{}.bin google-{}.bin.".format(label, label)
  subprocess.call(command.split(' '))
  print "done %s" % label

labels = ngram_labels
cores = multiprocessing.cpu_count()
Parallel(n_jobs=cores)(delayed(do)(i) for i in labels)
