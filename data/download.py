...
def run(label):
  url = ngram_url.format(label)
  gz_path  = "{}.gz".format(label)
  bin_path = "{}-{}.bin".format("google", label)
  ...
  try:
    if not os.path.exists(bin_path):
      if not os.path.exists(label):
        ...
        subprocess.call("pigz -d {}".format(gz_path).split(' '))
      print "moving", label
      subprocess.call("mv {} {}".format(label, bin_path).split(' '))

    print bin_path
  except Exception as e:
    print "Had problems with", label
    print e
...
# Parallel(n_jobs=cores)(delayed(do)(i) for i in ngram_labels)
